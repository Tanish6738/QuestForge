...
async function generateQuestsWithAI(goal, questType = 'main', count = 3) {
  try {
    if (!process.env.GEMINI_API_KEY || API_KEY) {
  ...
  try {
    ...
    for (let day = 1; day <= Math.min(totalDays, 30); day++) { // Limit to 30 days initially to prevent timeouts
      ...
      for (const questData of dailyQuests) {
        // Validate and sanitize quest type
        let questType = questData.type;
        if (!['main', 'sub', 'side'].includes(questType)) {
          // Map 'bonus' or any invalid type to 'side' if optional, else 'main'
          questType = questData.isOptional ? 'side' : 'main';
        }
...
export async function getTodaysQuests(userId) {
  try {
    ...
    // First, find quests scheduled for today
    const scheduledQuests = await Quest.find({
      userId,
      scheduledDate: {
        $gte: today,
        $lte: todayEnd
    ...
    if (scheduledQuests.length > 0) {
      console.log(`✅ Found ${scheduledQuests.length} quests scheduled for today`);
      allQuests.push(...scheduledQuests);
    }
    
    // Then, find quests from active quest plans for today
    const questPlans = await QuestPlan.find({ 
      userId, 
      status: { $in: ['draft', 'active'] }
    });
    
    if (questPlans.length > 0) {
      console.log(`📋 Found ${questPlans.length} active quest plans, checking for today's quests.`);
      for (const plan of questPlans) {
        ...
        if (todaysPlan && todaysPlan.quests) {
          ...
          if (todaysQuests.length > 0) {
            console.log(`✅ Found ${todaysQuests.length} quests in plan "${plan.title}"`);
            allQuests.push(...todaysQuests);
          }
        }
      }
    }
    
    // De-duplicate the combined list of quests
    const uniqueQuests = Array.from(new Map(allQuests.map(q => [q._id.toString(), q])).values());

    if (allQuests.length !== uniqueQuests.length) {
      console.warn(`⚠️ Duplicate quests found and removed. Before: ${allQuests.length}, After: ${uniqueQuests.length}`);
    }
